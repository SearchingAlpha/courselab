...
const authOptions = {
  ...
  cookies: {
    name: 'sb-wwgcghsoogmsmpeseszw', // Must match the project reference in the cookie name
...
export const supabase = createClient(
  ...
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY,
  {
    auth: {
      ...
      persistSession: false, // Server shouldn't persist session
      autoRefreshToken: false, // Let client handle token refresh
    },
    global: {
...
export async function getSession() {
  try {
    ...
    if (!data?.session) {
      // Try getting the session from cookies directly
      // This is a fallback mechanism
      const cookieStr = typeof document !== 'undefined' ? document.cookie : '';
      const hasAuthCookie = cookieStr.split(';')
        .map(c => c.trim())
        .some(c => c.startsWith('sb-access-token='));
        
      if (hasAuthCookie) {
        console.log("Auth: Session not found but auth cookie exists. Refreshing auth state...");
        
        // Attempt a getUser call which might help refresh the session
